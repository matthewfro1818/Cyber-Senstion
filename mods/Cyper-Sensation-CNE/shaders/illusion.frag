...
void mainImage() {
    ...
    vec3 clr;
    clr.r = texture(iChannel0, uv2 + vec2(r_shift.x, r_shift.y)).r;
    clr.g = texture(iChannel0, uv2 + vec2(g_shift.x, g_shift.y)).g;
   	clr.b = texture(iChannel0, uv2 + vec2(b_shift.x, b_shift.y)).b;
    fragColor = vec4(clr,flixel_texture2D(bitmap, uv2).a);   
}