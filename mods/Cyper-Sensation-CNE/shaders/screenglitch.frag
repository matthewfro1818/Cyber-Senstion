...
void mainImage()
{
    ...
    vec2 a = vec2(uv1.x * (iResolution.x / iResolution.y), uv1.y);
    ...
    float scanline = abs(cos(uv1.y * 400.));
    scanline = smoothstep(0.0, 2.0, scanline);
    shift = smoothstep(0.00001, 0.2, shift);
    
    // Apply glitch and RGB shift
    float colR = texture(iChannel0, vec2(uv1.x + shift, uv1.y)).r * (1. - shift) ;
    float colG = texture(iChannel0, vec2(uv1.x - shift, uv1.y)).g * (1. - shift) + rand(id) * shift;
    float colB = texture(iChannel0, vec2(uv1.x - shift, uv1.y)).b * (1. - shift);
    // Mix with the scanline effect
    vec3 f = vec3(colR, colG, colB) - (0.1 * scanline);
    
    // Output to screen
    fragColor = vec4(f, flixel_texture2D(bitmap, uv1).a);
}