...
function playvideo(videoname) {
	trace('playvide');
	if (video.load('mods/' + ModsFolder.currentModFolder + '/videos/CH2/' + videoname + '.mp4'))
...
function create() {
	...
	FlxG.save.data.lasthope = false;
	FlxG.save.flush();
	trace(FlxG.save.data.lasthope);
  if(FlxG.save.data.useSubs) {usesub = true;}
	if(FlxG.save.data.thaiDub) {dublang = '-th';}
  if(FlxG.save.data.thaiSub) {sublang = '-th';}
  jsonString = Paths.file('videos/CH2/' + videoname + sublang + '.json');
	...
	cutva = FlxG.sound.load(Paths.sound('cutscene/ch2/' + videoname + dublang), 1);
